...
- **Say "ANVI"** (or click the orb / press Space) — wake up. It keeps listening, detects when you stop talking, replies, and listens again.
...
Wake-word detection while asleep uses the browser's built-in speech recognition, so it works in Edge and Chrome.
